...
function randomFromCoordinates(row, column, salt) {
  let seed = Math.imul(row + 1, 73856093) ^ Math.imul(column + 1, 19349663) ^ salt
  ...
  return (
    <div className={className} style={gridStyle} aria-hidden>
      {coords.map(([r, c], i) => {
        ...
        const startY = -220 - randomFromCoordinates(r, c, 1) * 180
        const startRot = randomFromCoordinates(r, c, 2) * 10 - 5
        const key = `${r},${c}`
          ...
          <motion.div
            ...
            initial={
              prefersReducedMotion
                ? false
                : { y: startY, rotate: startRot, opacity: 0, scale: 0.96 }
            }
