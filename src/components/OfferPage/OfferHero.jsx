...
export default function OfferHero({ onContact }) {
  const { t } = useI18n()
    ...
    <section className="bg-bg-1 text-white section-wrapper relative overflow-hidden">
      <div className="section-inner grid lg:grid-cols-12 gap-14 lg:gap-10 items-end">
        <div className="lg:col-span-7">
          <motion.p {...fadeUp(0)} className="text-xs tracking-widest text-gray-500 mb-4 uppercase">
          ...
          >
            <DefaultButton label={t('offerPage.hero.cta')} onClick={onContact} />
          ...
          <ol className="border-t border-bg-3/70">
            {OFFER_SERVICES.map(({ anchor, key }, i) => (
              <li key={anchor} className="border-b border-bg-3/70">
                <a
                  href={`#${anchor}`}
                  className="group flex items-center gap-4 py-4 transition-colors duration-200 hover:text-primary"
                >
                  <span className="text-xs font-semibold text-primary tabular-nums w-6">
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <span className="flex-1 font-medium">{t(`offerPage.services.${key}.title`)}</span>
                  <ArrowDownRight
                    aria-hidden="true"
                    className="w-4 h-4 text-gray-500 transition-transform duration-200 ease-[var(--ease-out-quart)] group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:text-primary"
                  />
                </a>
              </li>
            ))}
