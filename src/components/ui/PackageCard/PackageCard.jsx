...
export default function PackageCard({ pkg, recommendedLabel, ctaLabel, onContact }) {
  return (
    <div
      className={`relative flex flex-col p-6 border overflow-hidden transition-all duration-300 ease-[var(--ease-out-quart)] hover:-translate-y-1 ${
        pkg.recommended
    ...
    >
      {pkg.recommended && (
        <div className="absolute top-0 right-0 w-36 h-36 pointer-events-none">
          ...
          >
            <span className="relative z-10">{recommendedLabel}</span>
            <span
      ...
      <p className="text-sm text-gray-400 mt-1 mb-5 pr-10">{pkg.summary}</p>
      ...
      <DefaultButton label={ctaLabel} onClick={onContact} className="w-full text-center text-sm" />
    </div>
  );
}
